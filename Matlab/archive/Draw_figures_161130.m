   %--------------------Figures-----------------------------------------------
   show_fig_number=[1 1 1 1 1 1 0 0];
   ...
   for side=1:4
     ...
     elseif side==2
           ...
           Side_A_conv_hull_Fi2_dark=Down_A_conv_hull;
     elseif side==3
           X_SIDE=X_LEFT;
           Y_SIDE=Y_LEFT;
           Fi1_SIDE=Fi1_LEFT;
     ...
     elseif side==4
      ...
      if side==1
           ...
           if show_fig_number(1)==1
               ...
               set(aa,'edgecolor','none','facecolor','interp')
               aa=surf(ones(length(Y_SIDE),1)*X_SIDE'+2*(1000*DX+20),Y_SIDE*ones(1,length(X_SIDE)),ones(length(Y_SIDE),length(X_SIDE))*B*1000,pith_angle_side_degrees_unsmoothed);
               set(aa,'edgecolor','none','facecolor','interp')
               ...
               contour(ones(length(Y_raw),1)*X_raw'+(1000*DX+20)*4,Y_raw*ones(1,length(X_raw)),mark_Side_dark_WE,1,'k')
               [nr,nk]=size(knot_data_min_max_side);
               for r=1:nr
                   knot_data_min_max_side(r,:);
                   plot(knot_data_min_max_side(r,[1 2 2 1 1])+(1000*DX+20)*4,knot_data_min_max_side(r,[3 3 4 4 3]),'-r');
               end
               colorbar
               axis equal
           end

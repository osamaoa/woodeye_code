 for series=1
    ...
    if series==1
        dir_project='M:\WoodEye5_NyStruktur\Caroline_Musaab_Osama_Testscan_Spruce2\raw'; % Rådata
        Parentdir='M:\WoodEye5_NyStruktur\Caroline_Musaab_Osama_Testscan_Spruce2\organized';          % Mapp för omorganiserad data
    else
    end
    ...
    nboards=length(files)/28;
    files=reshape(files,nboards,28);
    ...
    for k=1:nboards
        [series k]
        if index(k)<10
            bdir=sprintf('B_00%d',index(k));
        elseif index(k)<100
            bdir=sprintf('B_0%d',index(k));
        elseif index(k)<1000
            bdir=sprintf('B_%d',index(k));
        end
        Parentdir_b=sprintf('%s%s%s',Parentdir,'\',bdir);
        
%         % OBS, raderna nedan ska bara användas i särskilda fall för att ta bort felaktigt genererade filer. VAR FÖRSIKTIG!
%         dir_to_empty_if_exist=sprintf('%s%s%s',Parentdir_b,'\','D_raw');
%         WD=cd;
%         cd(dir_to_empty_if_exist)
%         delete *.txt
%         delete *.raw
%         cd(WD);
        %---------------------------------------------------------------------------------------------------

        [SUCCESS,MESSAGE,MESSAGEID]=mkdir(Parentdir_b,'D_raw');
        for j=1:28
            fn=char(files(k,j));
            fno=fn;
            lfn=length(fn);
            ...
            destination=sprintf('%s%s%s%s%s',Parentdir_b,'\','D_raw','\',fn,'.raw');
